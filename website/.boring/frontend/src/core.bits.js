...
export async function buildBundleLibrary ( configuration, dev = false )
{
    ...
    const scriptPath = Path.join( getRootPath(), libraryPath, "index.js"   );
    const stylePath  = Path.join( getRootPath(), libraryPath, "index.css"  );

    const scriptName   = dev ? `index.js`  : `index.[hash].js`;
    const styleName    = dev ? `index.css` : `index.[hash].css`;

    const outputScript = await compileBundle( scriptPath, `${ libraryBuild }/${ scriptName }`, outputPath, libraryPath, dev );
    const outputStyle  = await compileBundle( stylePath , `${ libraryBuild }/${ styleName }` , outputPath, libraryPath, dev );
...
export async function buildNativeLibrary ( configuration, dev = false )
{
    ...
    for ( const file in content )
    {
        ...
        isStyleFile ( file ) && ( compiled = await compileStyle ( param, fileMappings.styles, dev ) );
...
export async function compileBundle ( entryPoint, entryName, outputPath, libraryPath, dev = false )
{
    ...
    if (
        isScriptFile( entryName ) ||
        isWorkerFile( entryName ) ||
        isTestFile  ( entryName )
    ) {
 ...
 * @return { code : string, map : string }    Object containing compiled code and a source map.
 */
export async function compileStyle ( file = {}, importMap = null, dev = false )
{
...
export function getE2ELocation ( configuration )
{
    const hostname = configuration.tests.e2eHostname;
    const port     = configuration.tests.e2ePort;
...
export function isScriptFile ( file )
{
    ...
    if ( file.includes( "test.utilities.js" ) )
    {
        return false;
    }
...
export async function registerPartials ( Handlebars, configuration )
{
    const
    {
        indexTemplate,
...
export function validateConfiguration ( configuration )
{
    ...
    /* Optional fields */
    const typeofDataFile = typeof configuration.dataFile;
    if ( typeofDataFile !== undefined && typeofDataFile !== "string" )
        throw new Error( `Configuration: 'dataFile' must be a string!` );

    const typeofNativeDependencies = typeof configuration.nativeDependencies;
    if ( typeofNativeDependencies !== undefined && typeofNativeDependencies !== "object" )
...
export async function writeViews ( Handlebars, configuration, fileMappings )
{
