...
/**
 * Return content of the `configuration.json` file.
 */
function getConfiguration ()
{
    const resolved      = import.meta.resolve( "../configuration.json" );
    const filePath      = nodeURL.fileURLToPath( resolved );
    const configuration = JSON.parse( FSSync.readFileSync( filePath, { encoding: "utf8" } ) );
...
async function buildStyles ( configuration, additionalMappings, dev = false )
{
    ...
    {
        const param    = { name : file, content : content[ file ] }
        const compiled = await Bits.compileStyle( param, additionalMappings, dev );
        const output   = fileMappings[ file ];
...
{
    const { assetsPath, buildPath } = configuration;
...
async function generateHTML ( configuration, loopState )
{
    await Bits.registerHelpers ( Handlebars, configuration, loopState );
    await Bits.registerPartials( Handlebars, configuration            );
...
{
    const { buildPath, devPort } = configuration;
    const fullBuildPath = Path.join( Bits.getRootPath(), buildPath );
...
{
    const { assetsPath, sourcePath } = configuration;
...
const tests =
{
    getE2ELocation ()
    {
        ...
        return Bits.getE2ELocation( configuration );
    },

    async runE2E ( configuration )
    {
        const
        {
            buildPath,
        ...
        const units  = await Bits.getTestFiles( root, ".e2e." );
    ...
    async runWebBrowser ( configuration )
    {
        const
        {
            buildPath,
        ...
        const root            = Path.join( Bits.getRootPath(), sourcePath );
        const units           = await Bits.getTestFiles( root, ".test." );
    ...
    cypress :
    {
        async runComponentTests ( configuration )
        {
            console.log( "cypress.runComponentTests" );
        },
        runE2E ( configuration )
        {
            const
            {
                buildPath,
            ...
            server.listen( tests.e2ePort, async () =>
            {
                console.info( `Server listening on port ${ tests.e2ePort }...` );
